...
class UserDAO
{
    ...
    public function deleteUser(int $id): void
    {
        try {
            ...
            // 1. Apagar filhos de screen_alert_displays do próprio user
            $sql = "DELETE a FROM alerts a 
                INNER JOIN screen_alert_displays s ON a.id_screen_alert_display = s.id 
                WHERE s.id_user = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            // 2. Apagar filhos de schedule_medications do próprio user
            $sql = "DELETE d FROM day_for_medications d 
                INNER JOIN schedule_medications sm ON d.id_schedule_medication = sm.id 
                WHERE sm.id_user = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            // 3. Apagar filhos de screen_alert_displays dos pacientes
            $sql = "DELETE a FROM alerts a 
                INNER JOIN screen_alert_displays s ON a.id_screen_alert_display = s.id 
                INNER JOIN users u ON s.id_user = u.id 
                WHERE u.id_cuidador = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            // 4. Apagar filhos de schedule_medications dos pacientes
            $sql = "DELETE d FROM day_for_medications d 
                INNER JOIN schedule_medications sm ON d.id_schedule_medication = sm.id 
            ...
            // 5. Apagar diretos do próprio user
            $sql = "DELETE FROM email_verifications WHERE id_user = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $sql = "DELETE FROM messages WHERE id_user = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $sql = "DELETE FROM schedule_medications WHERE id_user = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $sql = "DELETE FROM screen_alert_displays WHERE id_user = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            // 6. Apagar diretos dos pacientes
            $sql = "DELETE e FROM email_verifications e INNER JOIN users u ON e.id_user = u.id WHERE u.id_cuidador = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $sql = "DELETE m FROM messages m INNER JOIN users u ON m.id_user = u.id WHERE u.id_cuidador = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $sql = "DELETE s FROM schedule_medications s INNER JOIN users u ON s.id_user = u.id WHERE u.id_cuidador = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            $sql = "DELETE s FROM screen_alert_displays s INNER JOIN users u ON s.id_user = u.id WHERE u.id_cuidador = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            // 7. Apagar pacientes do cuidador
            $sql = "DELETE FROM users WHERE id_cuidador = :id";
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute();

            // 8. Apagar o próprio user
            ...
            $this->conn->rollBack();
            throw new Exception("Erro ao apagar user: " . $e->getMessage());
        }
    }
}